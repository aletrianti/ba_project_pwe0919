...
import { SET_TASK_FOUR_AS_COMPLETED, SET_TASK_ONE_AS_COMPLETED, SET_TASK_THREE_AS_COMPLETED, SET_TASK_TWO_AS_COMPLETED } from "../store/actions/tasks/tasks.types";
...
export const storeTasksDeadlines = async () => {
    ...
    tasks.forEach(task => {
        if (task.taskID === 1) store.dispatch({ type: SET_TASK_ONE_AS_COMPLETED, payload: {...store.getState().taskOne, deadline: task.deadline} });
        if (task.taskID === 2) store.dispatch({ type: SET_TASK_TWO_AS_COMPLETED, payload: {...store.getState().taskTwo, deadline: task.deadline} });
        if (task.taskID === 3) store.dispatch({ type: SET_TASK_THREE_AS_COMPLETED, payload: {...store.getState().taskTwo, deadline: task.deadline} });
        if (task.taskID === 4) store.dispatch({ type: SET_TASK_FOUR_AS_COMPLETED, payload: {...store.getState().taskFour, deadline: task.deadline} });
    });
}
